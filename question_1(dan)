...
def format_time(elapsed_time):
  if elapsed_time < 1:
      return f"{round(elapsed_time * 1000, 2)} milliseconds"
  else:
      return f"{round(elapsed_time, 2)} seconds"

# Data initialization
command_list = ['addpassenger', 'addflight', 'loadfile', 'flightlist', 'checkflight', 'passengerlist', 'viewpassenger',
                'flightrates', 'generatedata', 'assignpassengers', 'help', 'exit', '']
command = ''
...
while command != 'exit':  # Repeats body until exit command is given
    ...
    if command == "addpassenger":  # Add new passenger to passenger list
        ...
        print(f"File data loaded! System finished in {format_time(elapsed_time)}\n")
        ...
        for i in range(len(flight_list)):
            ...
            flight_rate_dict[f_number] = str(
                round(float(pax_n) / float(cap) * 100, 2)) + "%"  # Calculate flight rate and store in dict

        flight_rate_dict_sorted = dict(
            sorted(flight_rate_dict.items(), key=lambda item: item[0]))  # Sort dictionary values in descending
        ...
        print(f"Passengers assigned! System finished in {format_time(elapsed_time)}\n")
        ...
        exit_confirm = input(
            "To confirm you wish to exit, enter \"CONFIRM\": ")  # Confirms users request to exit program
